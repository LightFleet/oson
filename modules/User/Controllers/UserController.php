...
class UserController extends FrontendController
{
    ...
    public function handleNewLogin($login){
        dd($login);
    }
    ...
    public function handleLogin(Request $request){
        ...
        if(strpos($login, '@') !== false){
            $user = User::whereEmail($login)->get()->first();
            if($user){
                return [
                    'status' => 'ok',
                    'phone' => 'no',
        ...
        $filtered_login = filter_var($login, FILTER_SANITIZE_NUMBER_INT);
        $login_to_check = str_replace("-", "", $filtered_login);

        if (strlen($login_to_check) < 12 || strlen($login_to_check) > 13) {
            return [
                'status' => 'not ok',
            ];
        }
        ...
        $user = User::wherePhone($login)->get()->first();
        if($user){
            return [
                'status' => 'ok',
                'phone' => 'yes',
